...
def main(cfg: DictConfig, override_args=None):
    ...
    #model, saved_cfg, task = checkpoint_utils.load_model_and_task(
    #checkpoint_path,
    #override_args=dict(cfg),
    #)
    model, saved_cfg, task = checkpoint_utils.load_model_and_task(cfg.common_eval.path)
                    ...
                    if sample["valid_classes"][i].tolist() == lead_class_indices:
                        for j in range(12):
                            ...
                            if f"{sample['ecg_id'][i][0]}_{j}" in examplar_buffers:
                                ...
                                statements = "\n".join([f"{grounding_classes.get(i.item(), f'Unknown({i.item()})')}: {scores[i].item():.3f}" for i in outputs])
                                statements += "\n\n"
